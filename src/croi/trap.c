...
{
    switch (cause & SCAUSE_CAUSE_MASK) {
    case 1: return "supervisor software";
    case 5: return "supervisor timer";
    case 9: return "supervisor external";
    default: return "(unknown)";
    }
}
...
void Croi_TrapDispatch(struct TrapFrame *frame)
{
    ...
    Croi_Print("\n*** UNHANDLED EXCEPTION ***\n"
               ...
               exception_name(cause), cause, frame->sepc, frame->stval,
               frame->sstatus, frame->x[1], frame->x[2], frame->x[3],
               frame->x[4], frame->x[10], frame->x[11], frame->x[12],
               frame->x[13]);
    Croi_Halt();
}
