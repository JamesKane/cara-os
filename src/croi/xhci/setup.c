...
[[nodiscard]] int Croi_Xhci_Setup(struct XhciController *c)
{
    ...
    if ((sts & XHCI_USBSTS_HCH) == 0) {
        LOG_ERROR("xhci", "Setup called while controller running (USBSTS=0x%x)",
                  (unsigned)sts);
        return CARA_EINVAL;
    }
    ...
    if (c->page_size_bytes != 4096) {
        LOG_ERROR("xhci", "PAGESIZE=%u; Phase 1 only handles 4 KiB",
                  (unsigned)c->page_size_bytes);
        return CARA_ERANGE;
    }

    // 1. CONFIG.MaxSlotsEn = max_slots.
    xhci_op_write32(c, XHCI_OP_CONFIG,
                    c->max_slots & XHCI_CONFIG_MAXSLOTSEN_MASK);
    ...
    xhci_op_write64_pair(c, XHCI_OP_DCBAAP_LO, XHCI_OP_DCBAAP_HI,
                         c->dcbaa_phys);
    ...
    xhci_rt_write64_pair(c,
                         ...
                         c->event_ring_phys);
    xhci_rt_write64_pair(c,
                         XHCI_RT_INTR0_BASE + XHCI_RT_INTR_ERSTBA_LO,
                         XHCI_RT_INTR0_BASE + XHCI_RT_INTR_ERSTBA_HI,
                         c->erst_phys);
    ...
    if (!xhci_spin_for_mask((volatile u32 *)(c->op_regs + XHCI_OP_USBSTS),
                            XHCI_USBSTS_HCH, 0)) {
    ...
    c->running = true;
    LOG_INFO("xhci",
             "running. DCBAA=0x%llx CmdRing=0x%llx EventRing=0x%llx ERST=0x%llx",
             (u64)c->dcbaa_phys, (u64)c->cmd_ring_phys,
             (u64)c->event_ring_phys, (u64)c->erst_phys);
    if (c->n_scratchpad_bufs > 0) {
        LOG_INFO("xhci", "scratchpad: array=0x%llx (%u bufs of %u bytes each)",
                 (u64)c->scratchpad_array_phys,
                 (unsigned)c->n_scratchpad_bufs,
                 (unsigned)c->page_size_bytes);
    }
    return CARA_EOK;
}
