...
static const char *dispatch_name(XhciInterfaceDispatch d)
{
    ...
    case XHCI_HID_MOUSE:
        return "HID/Boot/Mouse";
    case XHCI_HID_OTHER:
        return "HID-other";
    case XHCI_DISP_UNSUPPORTED:
        return "unsupported";
    default:
        return "none";
    }
    ...
    for (u32 sid = 1; sid <= CARA_XHCI_MAX_SLOTS; sid++) {
        ...
        for (u32 j = 0; j < c->slots[sid].n_interfaces; j++) {
            ...
            if (d == XHCI_DISP_UNSUPPORTED) {
                LOG_INFO("xhci", "slot=%u iface[%u] class=%u/%u/%u: unsupported in Phase 1",
                         (unsigned)sid, (unsigned)j, (unsigned)iface->bInterfaceClass,
                         (unsigned)iface->bInterfaceSubClass, (unsigned)iface->bInterfaceProtocol);
            } else {
                LOG_INFO("xhci", "slot=%u iface[%u] dispatch=%s", (unsigned)sid, (unsigned)j,
                         dispatch_name(d));
            }
        }
    }

    LOG_INFO("xhci", "dispatch totals: HID keyboards=%u HID mice=%u", (unsigned)c->n_hid_keyboards,
