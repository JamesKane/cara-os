...
KERNEL_TEST(handle_smoke)
{
    ...
    for (u32 i = 0; i < N; i++) {
        Kobj_Init(&g_kobjs[i], (i & 1) ? KOBJ_SIGNAL : KOBJ_TASK,
                  /*destroy=*/nullptr);
    }

    // Open all N handles.
    for (u32 i = 0; i < N; i++) {
        TEST_ASSERT(ctx, HandleTable_Open(ht, &g_kobjs[i], &g_hs[i]) == CARA_EOK, "open failed");
    }
    ...
    for (u32 i = 0; i < N; i++) {
        ...
        TEST_ASSERT(ctx, HandleTable_Lookup(ht, g_hs[i], expected, &k) == CARA_EOK,
    ...
    {
        struct Kobj *k = nullptr;
        TEST_ASSERT(ctx, HandleTable_Lookup(ht, g_hs[0], KOBJ_MSGPORT, &k) == CARA_EBADF,
                    "type mismatch should EBADF");
    }

    // Refcount: each open bumped from 1 to 2.
    for (u32 i = 0; i < N; i++) {
        TEST_ASSERT(ctx, g_kobjs[i].refcount == 2u, "refcount not bumped on Open");
    }

    // Close half of the handles.
    for (u32 i = 0; i < N / 2; i++) {
        TEST_ASSERT(ctx, HandleTable_Close(ht, g_hs[i]) == CARA_EOK, "close failed");
    }

    // Closed half: refcount returned to 1, lookups now return EBADF.
    for (u32 i = 0; i < N / 2; i++) {
        TEST_ASSERT(ctx, g_kobjs[i].refcount == 1u, "refcount not decremented on Close");
        struct Kobj *k = nullptr;
        TEST_ASSERT(ctx, HandleTable_Lookup(ht, g_hs[i], KOBJ_NONE, &k) == CARA_EBADF,
    ...
    for (u32 i = 0; i < N / 2; i++) {
        TEST_ASSERT(ctx, HandleTable_Open(ht, &g_kobjs[i], &g_hs2[i]) == CARA_EOK, "reopen failed");
        TEST_ASSERT(ctx, g_hs2[i] != g_hs[i], "handle reused without generation bump");
    }
    ...
    for (u32 i = 0; i < N; i++) {
        TEST_ASSERT(ctx, g_kobjs[i].refcount == 1u, "refcount not restored after all closes");
    }
}
