...
static const u8 g_usage_to_rawkey[256] = {
    ...
    [0x00] = CARA_RAWKEY_NONE,
    [0x01] = CARA_RAWKEY_NONE,
    [0x02] = CARA_RAWKEY_NONE,
    [0x03] = CARA_RAWKEY_NONE,
    ...
    [0x44] = CARA_RAWKEY_NONE,
    [0x45] = CARA_RAWKEY_NONE,
...
{
    u16 q = 0;
    if (mod & HID_MOD_LSHIFT)
        q |= IEQUALIFIER_LSHIFT;
    if (mod & HID_MOD_RSHIFT)
        q |= IEQUALIFIER_RSHIFT;
    // Amiga collapses LCTRL/RCTRL into one CONTROL bit.
    if (mod & (HID_MOD_LCTRL | HID_MOD_RCTRL))
        q |= IEQUALIFIER_CONTROL;
    if (mod & HID_MOD_LALT)
        q |= IEQUALIFIER_LALT;
    if (mod & HID_MOD_RALT)
        q |= IEQUALIFIER_RALT;
    if (mod & HID_MOD_LGUI)
        q |= IEQUALIFIER_LCOMMAND;
    if (mod & HID_MOD_RGUI)
                                           ...
                                           struct CaraHidMouseReport *out)
{
    ...
    if (out->buttons & HID_MOUSE_BTN_LEFT)
        q |= IEQUALIFIER_LEFTBUTTON;
    if (out->buttons & HID_MOUSE_BTN_RIGHT)
        q |= IEQUALIFIER_RBUTTON;
    if (out->buttons & HID_MOUSE_BTN_MIDDLE)
        q |= IEQUALIFIER_MIDBUTTON;
    out->ie_qualifier = q;
    return CARA_EOK;
}
