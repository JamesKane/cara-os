...
enum : u16 {
    XHCI_CAP_CAPLENGTH = 0x00,  // u8
    XHCI_CAP_HCIVERSION = 0x02, // u16
    XHCI_CAP_HCSPARAMS1 = 0x04, // u32
    XHCI_CAP_HCSPARAMS2 = 0x08, // u32
    XHCI_CAP_HCSPARAMS3 = 0x0C, // u32
    XHCI_CAP_HCCPARAMS1 = 0x10, // u32
    XHCI_CAP_DBOFF = 0x14,      // u32 (lower 2 bits reserved)
    XHCI_CAP_RTSOFF = 0x18,     // u32 (lower 5 bits reserved)
    XHCI_CAP_HCCPARAMS2 = 0x1C, // u32
};

// HCSPARAMS1 field positions (xHCI 1.2 §5.3.3).
enum : u32 {
    XHCI_HCSP1_MAX_SLOTS_SHIFT = 0,
    XHCI_HCSP1_MAX_SLOTS_MASK = 0xFFu,
    XHCI_HCSP1_MAX_INTRS_SHIFT = 8,
    XHCI_HCSP1_MAX_INTRS_MASK = 0x7FFu,
    XHCI_HCSP1_MAX_PORTS_SHIFT = 24,
    XHCI_HCSP1_MAX_PORTS_MASK = 0xFFu,
};

// HCSPARAMS2.MaxScratchpadBuffers is split: high 5 bits at [25:21],
// low 5 bits at [31:27]. xHCI 1.2 §5.3.4.
enum : u32 {
    XHCI_HCSP2_SCR_HI_SHIFT = 21,
    XHCI_HCSP2_SCR_HI_MASK = 0x1Fu,
    XHCI_HCSP2_SCR_LO_SHIFT = 27,
    XHCI_HCSP2_SCR_LO_MASK = 0x1Fu,
};

enum : u32 {
    XHCI_HCCP1_AC64 = (1u << 0),
    XHCI_HCCP1_CSZ = (1u << 2), // 1 → 64-byte device contexts
};
...
enum : u16 {
    ...
    XHCI_OP_DNCTRL = 0x14,
    XHCI_OP_CRCR_LO = 0x18,
    XHCI_OP_CRCR_HI = 0x1C,
    XHCI_OP_DCBAAP_LO = 0x30,
    XHCI_OP_DCBAAP_HI = 0x34,
    XHCI_OP_CONFIG = 0x38,
};

constexpr u16 XHCI_OP_PORTSC_BASE = 0x400;
constexpr u16 XHCI_OP_PORTSC_STRIDE = 0x10;

// USBCMD bits.
enum : u32 {
    XHCI_USBCMD_RUN = (1u << 0),
    XHCI_USBCMD_HCRST = (1u << 1),
    XHCI_USBCMD_INTE = (1u << 2),
    XHCI_USBCMD_HSEE = (1u << 3),
};

// USBSTS bits.
enum : u32 {
    XHCI_USBSTS_HCH = (1u << 0),  // halted
    XHCI_USBSTS_HSE = (1u << 2),  // host system error
    XHCI_USBSTS_EINT = (1u << 3), // event interrupt
    XHCI_USBSTS_PCD = (1u << 4),  // port change detect
    XHCI_USBSTS_CNR = (1u << 11), // controller not ready
    XHCI_USBSTS_HCE = (1u << 12), // host controller error
};

// CONFIG.MaxSlotsEn occupies the low 8 bits.
constexpr u32 XHCI_CONFIG_MAXSLOTSEN_MASK = 0xFFu;

// Runtime register offsets (at RTSOFF, masked low 5 bits).
enum : u16 {
    XHCI_RT_MFINDEX = 0x0000,
    XHCI_RT_INTR0_BASE = 0x0020,  // Interrupter 0 register set
    XHCI_RT_INTR_STRIDE = 0x0020, // 32 bytes per interrupter
    XHCI_RT_INTR_IMAN = 0x00,
    XHCI_RT_INTR_IMOD = 0x04,
    XHCI_RT_INTR_ERSTSZ = 0x08,
    XHCI_RT_INTR_ERSTBA_LO = 0x10,
    XHCI_RT_INTR_ERSTBA_HI = 0x14,
};
enum : u16 {
    XHCI_RT_INTR_ERDP_LO = 0x18,
    XHCI_RT_INTR_ERDP_HI = 0x1C,
};

// CRCR control bits (xHCI 1.2 §5.4.5).
enum : u32 {
    XHCI_CRCR_RCS = (1u << 0), // Ring Cycle State (initial)
    XHCI_CRCR_CS = (1u << 1),  // Command Stop
    XHCI_CRCR_CA = (1u << 2),  // Command Abort
    XHCI_CRCR_CRR = (1u << 3), // Command Ring Running (RO)
};

// PORTSC bits (xHCI 1.2 §5.4.8.1).
enum : u32 {
    XHCI_PORTSC_CCS = (1u << 0), // Current Connect Status
    XHCI_PORTSC_PED = (1u << 1), // Port Enabled (RW1C)
    XHCI_PORTSC_OCA = (1u << 3), // Overcurrent Active
    XHCI_PORTSC_PR = (1u << 4),  // Port Reset (RW1S)
    XHCI_PORTSC_PLS_SHIFT = 5,
    XHCI_PORTSC_PLS_MASK = 0xFu,
    XHCI_PORTSC_PP = (1u << 9), // Port Power
    XHCI_PORTSC_SPEED_SHIFT = 10,
    XHCI_PORTSC_SPEED_MASK = 0xFu,

    // Change bits — RW1C, software clears by writing 1. To preserve
    // state when modifying other PORTSC fields, mask with RW1C_MASK.
    XHCI_PORTSC_CSC = (1u << 17), // Connect Status Change
    XHCI_PORTSC_PEC = (1u << 18), // Port Enable Change
    XHCI_PORTSC_WRC = (1u << 19), // Warm Port Reset Change
    XHCI_PORTSC_OCC = (1u << 20), // Overcurrent Change
    XHCI_PORTSC_PRC = (1u << 21), // Port Reset Change
    XHCI_PORTSC_PLC = (1u << 22), // Port Link State Change
    XHCI_PORTSC_CEC = (1u << 23), // Port Configuration Error Change
};

constexpr u32 XHCI_PORTSC_RW1C_MASK = (XHCI_PORTSC_PED | XHCI_PORTSC_CSC | XHCI_PORTSC_PEC |
...
enum : u32 {
    XHCI_SPEED_FULL = 1,       // USB 2.0 Full-speed (12 Mbps)
    XHCI_SPEED_LOW = 2,        // USB 2.0 Low-speed (1.5 Mbps)
    XHCI_SPEED_HIGH = 3,       // USB 2.0 High-speed (480 Mbps)
    XHCI_SPEED_SUPER = 4,      // USB 3.0 SuperSpeed (5 Gbps)
    XHCI_SPEED_SUPER_PLUS = 5, // USB 3.1 SuperSpeedPlus (10 Gbps)
};
...
constexpr u32 XHCI_TRB_BYTES = 16;
constexpr u32 XHCI_TRB_CYCLE = (1u << 0);
constexpr u32 XHCI_TRB_TYPE_SHIFT = 10;
constexpr u32 XHCI_TRB_TYPE_MASK = 0x3Fu;

static inline u32 XHCI_TRB_TYPE(u32 t)
{
    return (t & XHCI_TRB_TYPE_MASK) << XHCI_TRB_TYPE_SHIFT;
}
...
enum : u32 {
    ...
    XHCI_TRB_NOOP_COMMAND = 23,
};

// Transfer TRB control bits (xHCI 1.2 §6.4.1).
enum : u32 {
    XHCI_TRB_ENT = (1u << 1), // Evaluate Next TRB
    XHCI_TRB_CHAIN = (1u << 4),
    XHCI_TRB_IOC = (1u << 5), // Interrupt On Completion
    XHCI_TRB_IDT = (1u << 6), // Immediate Data Transfer
};

// Setup Stage TRB Transfer Type (TRT, control[17:16]).
enum : u32 {
    XHCI_TRB_TRT_NO_DATA = (0u << 16),
    XHCI_TRB_TRT_OUT_DATA = (2u << 16),
    XHCI_TRB_TRT_IN_DATA = (3u << 16),
};

// Data / Status Stage Direction bit (control[16]). 1 = IN (device->host).
constexpr u32 XHCI_TRB_DIR_IN = (1u << 16);
...
constexpr u32 XHCI_TRB_LINK_TC = (1u << 1); // Toggle Cycle
...
constexpr u32 XHCI_TRB_ADDRDEV_BSR = (1u << 9);

// Slot ID is encoded in control[31:24] for slot-targeted commands.
constexpr u32 XHCI_TRB_SLOT_ID_SHIFT = 24;
...
enum : u32 {
    XHCI_SLOT_CTX_DW0_SPEED_SHIFT = 20,
    XHCI_SLOT_CTX_DW0_CTXENTRIES_SHIFT = 27,
    XHCI_SLOT_CTX_DW1_RHPORT_SHIFT = 16,
    XHCI_SLOT_CTX_DW3_ADDRESS_MASK = 0xFFu,
    XHCI_SLOT_CTX_DW3_STATE_SHIFT = 27,
    XHCI_SLOT_CTX_DW3_STATE_MASK = 0x1Fu,
};
...
enum : u32 {
    XHCI_EP_CTX_DW0_INTERVAL_SHIFT = 16,
    XHCI_EP_CTX_DW1_CERR_SHIFT = 1,
    XHCI_EP_CTX_DW1_EPTYPE_SHIFT = 3,
    XHCI_EP_CTX_DW1_MAXPKT_SHIFT = 16,
    XHCI_EP_CTX_DW2_DCS = (1u << 0),
    XHCI_EP_CTX_DW4_AVGTRB_LEN_SHIFT = 0,
};
...
enum : u32 {
    XHCI_EP0_MAXPKT_LOW = 8,     // LS: always 8
    XHCI_EP0_MAXPKT_FULL = 8,    // FS: 8/16/32/64; 8 is safe
    XHCI_EP0_MAXPKT_HIGH = 64,   // HS: always 64
    XHCI_EP0_MAXPKT_SUPER = 512, // SS+: always 512
};
...
enum : u32 {
    XHCI_INPUT_CTX_ADD_SLOT = (1u << 0),
    XHCI_INPUT_CTX_ADD_EP0 = (1u << 1),
};

// ---- Driver state -----------------------------------------------------------

constexpr u32 CARA_XHCI_MAX_SLOTS = 32;
...
constexpr u32 CARA_XHCI_MAX_INTERFACES_PER_SLOT = 4;
...
constexpr u32 CARA_XHCI_MAX_CONFIG_BYTES = 256;
