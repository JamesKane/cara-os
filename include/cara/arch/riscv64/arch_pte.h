...
//
// RISC-V Sv39 page-table-entry encoding — one of the CARA_ARCH-selected
// variants behind <cara/arch_pte.h> (epic H.2/H.7.2b). These are pure bit-math
// `static inline`s with no privileged instructions, so they compile for both
// the host (cara_mm unit tests) and the kernel; the generic page-table walk in
// src/croi/mm/pt.c uses them so the walk itself stays arch-neutral. The
// privileged side of the MMU (satp/sfence activate + fence + boot-root read)
// lives in src/croi/arch/riscv64/mmu.c behind arch_mmu_* (cara/arch.h).

#ifndef CARA_ARCH_RISCV64_PTE_H
#define CARA_ARCH_RISCV64_PTE_H
